...
#to read the data into a mySQL table.  V3.0 unifies the previous version with protonpolisher to allow the
#use of proton VCFs.  Added code to decrease risk of SQL header collisions.
...
sub main(){
    ...
    LINE: while (my $line = <VCF>) { #starts a loop called line to read through lines of the VCF
        ...
        if ($line =~ /^\w/){  #if the line starts with an alphaneumeric character (or a line break)
            ...
            $hashalt =~ s/\W/\,/i;
            my $varMD5 = md5_hex($chromosome.":".$line[$columnposition{'POS'}]."/".$hashref."/".$hashalt);
            ...
            if (defined($columnposition{'Allelic_depths_for_the_ref_and_alt_alleles_in_the_order_listed'})) {
                ...
                if (defined($line[$columnposition{'Total_read_depth_at_the_locus'}])) {
                    $proton = 'TRUE';
                }
                
            }
            
            if ($line[$columnposition{'FORMAT'} + 1] !~ /^\.\/\./ and ($standard or $proton)) {  #checks for null reads
                ...
                if ($standard) {
                    ...
                    {
                    my @altreads = split (/,/, $line[$columnposition{'Alternate_allele_observations'}]);
                    ...
                    my $refreads = $line[$columnposition{'Total_read_depth_at_the_locus'}] - $totalaltreads;
                    $reads[0] = $refreads;
